...
from .tr import build_transformer
from .utils import mask_from_tensor


class GetPE(nn.Module):
    def __init__(self):
        super().__init__()

    def forward(self, position_encoding, z_):
...
class UGTRNet(nn.Module):
    def __init__(
        ...
        self.input_proj = nn.Sequential(
            nn.Conv2d(2048, self.hidden_dim, kernel_size=1, bias=False),
    ...
    def forward(self, x, y=None):
        ...
        if self.training:
            rand_mask = uncertainty < torch.Tensor(
                np.random.random(uncertainty.size())
            ).to(uncertainty.device)
        ...
        # x, mask = mask_from_tensor(x)
        # position_encoding = self.position_encoding(x, mask).to(x.device)
        position_encoding = self.get_pe(position_encoding, z_)
        # transformer
        x = self.transformer(
            x, residual, position_encoding
        )  # self.query_embed.weight, position_encoding, residual)
        x = self.pred(x)
        if self.zoom_factor != 1:
            prob_x = F.interpolate(
                prob_x, size=(h, w), mode="bilinear", align_corners=True
            )
            x = F.interpolate(x, size=(h, w), mode="bilinear", align_corners=True)
            mean3 = F.interpolate(
                mean3, size=(h, w), mode="bilinear", align_corners=True
            )
        ...
        if self.training:
            ...
            return (
                x,
                ((std3 - std3.min()) / (std3.max() - std3.min())),
                mean3,
            )  # , uncertainty
