...
from torch.distributions import Independent, kl, Normal
...
class Encoder_x(nn.Module):
    def __init__(self, input_channels, channels, latent_size):
        ...
        self.layer1 = nn.Conv2d(
            input_channels, channels, kernel_size=4, stride=2, padding=1
        )
        self.bn1 = nn.BatchNorm2d(channels)
        self.layer2 = nn.Conv2d(
            channels, 2 * channels, kernel_size=4, stride=2, padding=1
        )
        self.bn2 = nn.BatchNorm2d(channels * 2)
        self.layer3 = nn.Conv2d(
            2 * channels, 4 * channels, kernel_size=4, stride=2, padding=1
        )
        self.bn3 = nn.BatchNorm2d(channels * 4)
        self.layer4 = nn.Conv2d(
            4 * channels, 8 * channels, kernel_size=4, stride=2, padding=1
        )
        self.bn4 = nn.BatchNorm2d(channels * 8)
        self.layer5 = nn.Conv2d(
            8 * channels, 8 * channels, kernel_size=4, stride=2, padding=1
        )
...
class Generator(nn.Module):
    ...
    def reparametrize(self, mu, logvar):
        ...
        eps = Variable(eps)
        eps = eps.to(device=mu.device)
        return eps.mul(std).add_(mu)

    def forward(self, data, y=None, training=False):
        x = data["image"]
        depth = data["depth"]
        if training:
            self.posterior, muxy, logvarxy = self.xy_encoder(
                torch.cat((x, depth, y), 1)
            )
            ...
            self.prob_pred_post, self.depth_pred_post = self.sal_encoder(
                x, depth, z_noise_post
            )
            self.prob_pred_prior, self.depth_pred_prior = self.sal_encoder(
                x, depth, z_noise_prior
            )
            return (
                self.prob_pred_post,
                self.prob_pred_prior,
                lattent_loss,
                self.depth_pred_post,
                self.depth_pred_prior,
            )
        else:
...
class multi_scale_aspp(nn.Sequential):
    ...
    def __init__(self, channel):
        ...
        self.classification = nn.Sequential(
            nn.Dropout2d(p=0.1),
            nn.Conv2d(
                in_channels=channel * 6, out_channels=channel, kernel_size=1, padding=0
            ),
        )
...
class Saliency_feat_encoder(nn.Module):
    ...
    def __init__(self, channel, latent_dim):
        ...
        self.layer6 = self._make_pred_layer(
            Classifier_Module, [6, 12, 18, 24], [6, 12, 18, 24], 1, channel
        )
        ...
        self.layer_depth = self._make_pred_layer(
            Classifier_Module, [6, 12, 18, 24], [6, 12, 18, 24], 3, channel * 4
        )
