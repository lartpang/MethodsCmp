...
class JLModule(nn.Module):
    def __init__(self, backbone):
        ...
        cp.append(
            nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Conv2d(512, k, 3, 1, 1),
                nn.ReLU(inplace=True),
            )
        )
        cp.append(
            nn.Sequential(
                nn.Conv2d(2048, 512, 7, 1, 6, 2),
                nn.ReLU(inplace=True),
                nn.Conv2d(512, 512, 7, 1, 6, 2),
                nn.ReLU(inplace=True),
                nn.Conv2d(512, k, 3, 1, 1),
                nn.ReLU(inplace=True),
            )
    ...
    def load_pretrained_model(self, model_path):
        ...
        self.vgg_conv1.load_state_dict(
            torch.load("pretrained/vgg_conv1.pth"), strict=True
        )
...
class JL_DCF(nn.Module):
    def __init__(
        ...
        self.upsampling = nn.ModuleList(
            nn.ModuleList(upsampling[i]) for i in range(0, 4)
        )
