...
from .basicnet import ConcatNet, MutualNet
...
class MGLNet(nn.Module):
    def __init__(
        ...
        self.edge_cat = ConcatNet()  # concat low-level feature map to predict edge

        # cascade mutual net
        self.mutualnet0 = MutualNet(
            dim=self.dim, num_clusters=num_clusters, dropout=dropout
        )
        if stage == 1:
            self.mutualnets = nn.ModuleList([self.mutualnet0])
        elif stage == 2:
            self.mutualnet1 = MutualNet(
                dim=self.dim, num_clusters=num_clusters, dropout=dropout
            )
    ...
    def forward(self, x, y=None, iter_num=0, y2=None):
        ...
        for net in self.mutualnets:
            ...
            if self.zoom_factor != 1:
                coee = F.interpolate(
                    coee, size=(h, w), mode="bilinear", align_corners=True
                )
                cod = F.interpolate(
                    cod, size=(h, w), mode="bilinear", align_corners=True
                )
                if self.training:
