...
class BasicConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, BatchNorm=nn.BatchNorm2d, **kwargs):
        ...
        self.stdv = 1.0 / math.sqrt(in_channels)
...
class ConcatNet(nn.Module):
    def __init__(self, BatchNorm=nn.BatchNorm2d):
        ...
        self.conv1 = nn.Sequential(
            BasicConv2d(c1, c1, BatchNorm, kernel_size=3, padding=1),
            BasicConv2d(c1, c1, BatchNorm, kernel_size=1, padding=0),
        )
        self.conv2 = nn.Sequential(
            BasicConv2d(c2, c2, BatchNorm, kernel_size=3, padding=1),
            BasicConv2d(c2, c2, BatchNorm, kernel_size=1, padding=0),
        )
        self.conv3 = nn.Sequential(
            BasicConv2d(c3, c2, BatchNorm, kernel_size=3, padding=1),
            BasicConv2d(c2, c2, BatchNorm, kernel_size=1, padding=0),
        )
        self.conv4 = nn.Sequential(
            BasicConv2d(c4, c2, BatchNorm, kernel_size=3, padding=1),
            BasicConv2d(c2, c2, BatchNorm, kernel_size=1, padding=0),
        )
        ...
        self.conv5 = nn.Sequential(
            BasicConv2d(c, c2, BatchNorm, kernel_size=3, padding=1),
            BasicConv2d(c2, c2, BatchNorm, kernel_size=1, padding=0),
        )

    def forward(self, x1, x2, x3, x4):
        x1 = F.interpolate(
            x1, size=(self.h, self.w), mode="bilinear", align_corners=True
        )
        x1 = self.conv1(x1)

        x2 = F.interpolate(
            x2, size=(self.h, self.w), mode="bilinear", align_corners=True
        )
        x2 = self.conv2(x2)

        x3 = F.interpolate(
            x3, size=(self.h, self.w), mode="bilinear", align_corners=True
        )
        x3 = self.conv3(x3)

        x4 = F.interpolate(
            x4, size=(self.h, self.w), mode="bilinear", align_corners=True
        )
        x4 = self.conv4(x4)
...
class GraphNet(nn.Module):
    ...
    def gen_soft_assign(self, x, sigma):
        B, C, H, W = x.size()
        N = H * W
        soft_assign = torch.zeros(
            [B, self.node_num, N], device=x.device, dtype=x.dtype, layout=x.layout
        )
        for node_id in range(self.node_num):
            residual = (
                x.view(B, C, -1).permute(0, 2, 1).contiguous() - self.anchor[node_id, :]
            ).div(
                sigma[node_id, :]
            )  # + eps)
    ...
    def forward(self, x):
        ...
        eps = 1e-9
        nodes = torch.zeros(
            [B, self.node_num, C], dtype=x.dtype, layout=x.layout, device=x.device
        )
        for node_id in range(self.node_num):
            residual = (
                x.view(B, C, -1).permute(0, 2, 1).contiguous() - self.anchor[node_id, :]
            ...
            nodes[:, node_id, :] = residual.mul(
                soft_assign[:, node_id, :].unsqueeze(2)
            ).sum(dim=1) / (soft_assign[:, node_id, :].sum(dim=1).unsqueeze(1) + eps)
...
class MutualModule0(nn.Module):
    def __init__(self, dim, BatchNorm=nn.BatchNorm2d, dropout=0.1):
        ...
        self.conv = nn.Sequential(
            BasicConv2d(dim, dim, BatchNorm, kernel_size=1, padding=0)
        )
...
class ECGraphNet(nn.Module):
    def __init__(self, dim, BatchNorm=nn.BatchNorm2d, dropout=0.1):
        ...
        self.conv0 = nn.Sequential(
            BasicConv2d(self.dim, self.dim, BatchNorm, kernel_size=1, padding=0)
        )
        self.node_num = 32
        self.proj0 = GraphNet(self.node_num, self.dim, False)
        self.conv1 = nn.Sequential(
            BasicConv2d(2 * self.dim, self.dim, BatchNorm, kernel_size=1, padding=0)
        )
    ...
    def forward(self, x, edge):
        ...
        residual_x = (
            residual_x.permute(2, 3, 1, 0).view(b, c, self.node_num, h, w).contiguous()
        )
        ...
        x1 = (
            torch.cat((x1 - x2, x2), dim=3).permute(0, 3, 1, 2).contiguous()
        )  # b x n x 5 x (2c)
        x1 = self.conv1(x1)
...
class MutualModule1(nn.Module):
    def __init__(self, dim, BatchNorm=nn.BatchNorm2d, dropout=0.1):
        ...
        self.pred0 = nn.Conv2d(
            self.dim, 1, kernel_size=1
        )  # predicted edge is used for edge-region mutual sub-module
...
class MutualNet(nn.Module):
    def __init__(self, BatchNorm=nn.BatchNorm2d, dim=512, num_clusters=8, dropout=0.1):
        ...
        self.edge_proj0 = GraphNet(
            node_num=num_clusters, dim=self.dim, normalize_input=False
        )
        self.region_proj0 = GraphNet(
            node_num=num_clusters, dim=self.dim, normalize_input=False
        )
