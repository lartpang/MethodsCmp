...
class Integration(nn.Module):
    ...
    def forward(self, low_1, low_2, high_1, high_2, high_3):
        ...
        Conv5_ori = self.C5_relu(
            self.C5_bn(
                self.C5_conv(
                    torch.cat([Conv5_1, Conv5_2, Conv5_3, Conv5_4, Conv5_5], dim=1)
                )
            )
        )
        Conv5 = F.interpolate(
            Conv5_ori, scale_factor=4, mode="bilinear", align_corners=False
        )
        ...
        Conv4_ori = self.C4_relu(
            self.C4_bn(
                self.C4_conv(
                    torch.cat([Conv4_1, Conv4_2, Conv4_3, Conv4_4, Conv4_5], dim=1)
                )
            )
        )
        Conv4 = F.interpolate(
            Conv4_ori, scale_factor=4, mode="bilinear", align_corners=False
        )
        ...
        Conv3 = self.C3_relu(
            self.C3_bn(
                self.C3_conv(
                    torch.cat([Conv3_1, Conv3_2, Conv3_3, Conv3_4, Conv3_5], dim=1)
                )
            )
        )
        ...
        Enhance_Feature = F.interpolate(
            Enhance_Feature, scale_factor=4, mode="bilinear", align_corners=False
        )
        Features = torch.cat([Enhance_Feature, Low], dim=1)

        high_depth = F.interpolate(
            high_depth, scale_factor=4, mode="bilinear", align_corners=False
        )
        high_sal = F.interpolate(
            high_sal, scale_factor=4, mode="bilinear", align_corners=False
        )
        pred_depth = F.interpolate(
            pred_depth, scale_factor=4, mode="bilinear", align_corners=False
        )
        pred_sal2 = F.interpolate(
            pred_sal2, scale_factor=4, mode="bilinear", align_corners=False
        )
...
class CoNet(nn.Module):
    def __init__(self):
        ...
        self.model_rgb = ResNet101(
            BatchNorm=nn.BatchNorm2d, pretrained=False, output_stride=16
        )
        self.model_intergration = Integration()
        self.model_att = TriAtt()

    def forward(self, data):
        assert isinstance(data, torch.Tensor)
        low_1, low_2, high_1, high_2, high_3 = self.model_rgb(data)
        Features, _, _, Edge, _, _, Depth, Sal = self.model_intergration(
            low_1, low_2, high_1, high_2, high_3
        )
        output = self.model_att(Features, Edge, Sal, Depth)
        return output
